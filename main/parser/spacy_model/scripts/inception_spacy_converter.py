...
def take_lower_or_equal(myList, myNumber):
    pos = bisect_left(myList, myNumber)
    if pos == 0:
        return myList[0]
    if pos == len(myList):
...
def convert_relation_ner_to_doc(typesysteme_filepath, xmi_filepath, split: int = 100, split_feature: str = "custom.Relation"):
    ...
    for segment in feature_split_idxs:
        ...
        for idx in range(len(tokens)):
            ...
            offset = doc[idx].idx - token_start
            spacy_offset[token_start] = offset
            spacy_offset[token_end] = offset
            if token_start+1 < token_end-1:
                for x in range(token_start+1, token_end-1, 1):
                    spacy_offset[x] = offset
            token_start_idx_map[token_start + spacy_offset[token_start]] =  idx
        # create list of keys for bisect 
        offset_keys = list(spacy_offset.keys())
